<template>
    <v-col>
        <v-toolbar color="primary" dark>
            <v-toolbar-title
                >{{
                    this.name.charAt(0).toUpperCase() + name.slice(1)
                }}
        ...
        <v-row>
            <v-container v-if="this.name == 'location' && centerr">
                <google-map
                    ...
                    :polylines-locations="this.polylines"
            ...
            <v-container
                ...
                "
            >
                <v-card class="pa-5" max-height="800" outlined tile>
                    <line-chart
                        :chart-data="datacollection_line"
                        :options="options"
                        :styles="myStyles"
                        v-if="loaded"
                    ></line-chart>
...
export default {
    ...
    methods: {
        ...
        getPropertyValues() {
            axios
                .get(`records/${this.recordId}/${this.name}`)
                .then(res => {
        ...
        configureTypeProperty: function(property) {
            ...
            if (property.dataType === 'NUMBER') {
                ...
                this.updates.forEach(update => {
                    update.value = parsing
                        .floatifyValue(update.value)
                        .toFixed(3);
                });
            }
            ...
            if (property.name === 'shock') {
                property.updates.forEach(property => {
                    ...
                    this.y_values.push(
                        (propertyValue.duration === 0
                            ? 0
                            : propertyValue.accel / propertyValue.duration
                        ).toFixed(3)
                    );
                    x_valuesAux.push(property.timestamp);
                });
            ...
            if (property.name === 'tilt') {
                property.updates.forEach(property => {
                    ...
                    this.y_values.push(
                        Math.sqrt(
                            propertyValue.x ** 2 + propertyValue.y ** 2
                        ).toFixed(3)
                    );
                    x_valuesAux.push(property.timestamp);
                });
        ...
        format(value) {
            if (
                ...
                this.name == 'origin' ||
                this.name == 'cultivationProcess' ||
                this.name == 'size' ||
                this.name == 'weight'
            ) {
                return value;
            } else if (
                this.name == 'harvestDate' ||
                this.name == 'packingDate' ||
            ...
            if (d.hasOwnProperty('accel')) {
                return (
                    'Accel: ' +
                    parsing.toFloat(d.accel) +
                    ', Duration: ' +
                    parsing.toFloat(d.duration)
                );
